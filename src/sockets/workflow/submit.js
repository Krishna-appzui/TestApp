...
/**
 * Socket Endpoint to submit a workflow.  Handles both submittion of create, and submittion of update.
 * @Author Bryan Nagle
 * @date 4/5/14
...
exports.handler = function(db, socket) {
    return function (data) {
        ...
        var response = require('../../../lib/core/socketResponse').response(socket, 'submitWorkflow', data);
        ...
        if (data.create) {
            createWorkflow(db, data, function(err, result) {
            ...
            wf.fetchStepsForWorkflow(data.workflow, function(steps) {
                updateWorkflow(db, data, steps, function(err, result) {

                    if (!_.isNull(err)) {
                        response(err, result);
                    } else if (!_.isUndefined(result) && result.last === true) {
                        // Last Step, send to backend!
                        interface.process(data.workflow, data.curr, function(err, result) {
                    ...
                    response('Error Saving Workflow', result);
                });
            });
        }
    }
